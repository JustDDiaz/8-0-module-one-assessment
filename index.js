...
function getHighestMetascore(movies) {
  ...
  for (let movie of movies) {
    if (stringToNum(movie.metascore) > highestMetascore) {
      highestMetascore = stringToNum(movie.metascore);
    }
  }
...
function getAverageIMDBRating(movies) {
  ...
  let divide = [];
  for (let movie of movies) {
    divide.push(movie.imdbRating);
    averageIMDBRating += stringToNum(movie.imdbRating);
  }
  if (averageIMDBRating === 0) {
    return 0;
  }
  return averageIMDBRating / divide.length;
}
...
function getBiggestBoxOfficeMovie(movies) {
  ...
  for (let i = 0; i < movies.length; i++) {
    let stringArr = movies[i].boxOffice.split("");
    stringArr.shift();
    let total = stringArr.join("");
